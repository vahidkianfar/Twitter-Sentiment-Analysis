...
class Program
{
    ...
    public static List<string> StartForBatchInput(string inputText)
    {
        var mlContext = new MLContext();
        var splitDataView = LoadData(mlContext);
        var model = BuildAndTrainModel(mlContext, splitDataView.TrainSet);
        var predictions = inputText.Split('\n').Select(line => ResultsFromModel(mlContext, model, line)).ToList();

        //return predictions.Aggregate((a, b) => a + "\n" + b);
        predictions.Add(Evaluate(mlContext, model, splitDataView.TestSet));
        return predictions;
    }
